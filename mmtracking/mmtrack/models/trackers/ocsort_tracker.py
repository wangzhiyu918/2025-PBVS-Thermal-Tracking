...
class OCSORTTracker(SortTracker):
    ...
    def ocm_assign_ids(self,
        ...
        if len(ids) > 0 and len(det_bboxes) > 0:
            ...
            if apply_weighted_matching:
                dists_oc = dists
                dists_thermal = self.get_thermal_dists_hist(track_bboxes, det_bboxes, self.cv_gray_image)

                if not dists_thermal.size == 0:
                    dists_thermal = (dists_thermal - dists_thermal.min()) / (dists_thermal.max() - dists_thermal.min())

                alpha = 0.8
                dists = (alpha * dists_oc) + ((1-alpha) * dists_thermal)
                match_iou_thr = match_iou_thr + weighted_shift
    ...
    def ocr_assign_ids(self,
        ...
        dists = dists_iou
        if apply_weighted_matching:
            dists_thermal = self.get_thermal_dists_hist(track_obs[:, :4], det_bboxes, self.cv_gray_image)
    ...
    def track(self,
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            match_track_inds, raw_match_det_inds = self.ocm_assign_ids(
                ...
                self.weight_iou_with_det_scores, self.match_iou_thr, apply_weighted_matching=False, weighted_shift=0.0)
            ...
            (tentative_match_track_inds,
             tentative_match_det_inds) = self.ocm_assign_ids(
                 ...
                 self.weight_iou_with_det_scores, self.match_iou_thr, apply_weighted_matching=False, weighted_shift=0)
            ...
            if len(unmatched_track_inds) > 0:
                ...
                _, ocr_match_det_inds = self.ocr_assign_ids(
                    ...
                    self.weight_iou_with_det_scores, self.match_iou_thr, apply_weighted_matching=False)
